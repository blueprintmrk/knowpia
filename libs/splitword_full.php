...
class SplitWord
{
    ...
    public $CnNumber  = "％|＋|－|０|１|２|３|４|５|６|７|８|９|．";
    public $CnSgNum   = "一|二|三|四|五|六|七|八|九|十|百|千|万|亿|数";
    public $MaxLen    = 13; //词典最大 7 中文字，这里的数值为字节数组的最大索引
    public $MinLen    = 3; //最小 2 中文字，这里的数值为字节数组的最大索引
    ...
    public function SplitRMM($str = "")
    {
        ...
        for ($i = ($spLen - 1); $i >= 0; $i--) {
            ...
            if ($this->NotGBK($spwords[$i])) {
                if (ereg("[^0-9\.\+\-]", $spwords[$i])) {$this->ResultString = $spwords[$i] . $spc . $this->ResultString;} else {
                    $nextword  = "";
                ...
                $n = hexdec(bin2hex($c));
                if ($c == "《") //书名
                { $this->ResultString = $spwords[$i] . $spc . $this->ResultString;} else if ($n > 0xA13F && $n < 0xAA40) //标点符号
                { $this->ResultString = $spwords[$i] . $spc . $this->ResultString;} else //正常短句
                {
    ...
    //----------------------------------------
    public function FindNewWord($spwords, $maxlen = 6)
    {
        $okstr   = '';
        $ws      = explode(' ', $spwords);
        $newword = '';
        ...
        foreach ($ws as $w) {
            $w = trim($w);
            if (strlen($w) == 2 && !preg_match("/[0-9a-zA-Z]/", $w) && !preg_match("/" . $this->NewWordLimit . "/", $w)) {$newword .= " " . $w;} else {
                if ($newword != "") {
    ...
    //--------------------------------------------------
    public function GetIndexText($okstr, $ilen = -1)
    {
        if ($okstr == "") {
            return "";
        }
